    ...
    def __init__(self, base_path, site, train=True, transform=None):
        if train:
            self.paths, self.text_labels = np.load(
                "./data/office_caltech_10/{}_train.pkl".format(site), allow_pickle=True
            )
        else:
            self.paths, self.text_labels = np.load(
                "./data/office_caltech_10/{}_test.pkl".format(site), allow_pickle=True
            )
        ...
        # logger.info("Total # labels:{}".format(len(self.labels)))
...
class FEMNIST(MNIST):
    ...
    def _check_exists(self) -> bool:
        return all(
            check_integrity(os.path.join(self.raw_folder, os.path.splitext(
                os.path.basename(url))[0]+os.path.splitext(os.path.basename(url))[1]))
...
class CheXpertDataset(Dataset):
    def __init__(self, dataset_type, data_np, label_np, pre_w, pre_h, lab_trans=None, un_trans_wk=None, data_idxs=None,
        ...
        logger.info('Total # images:{}, labels:{}'.format(len(self.images), len(self.labels)))
...
class TransformTwice:
    def __init__(self, weak_trans, strong_trans):
        ...
        self.st_transform = strong_trans
